...
_NOOP_HINTS: dict[str, str] = {
    "loop_region": (
        "Prefer selection master_bar_range (converted to song-local source_*_bar). "
        "Default paste_mode after_selection pastes at source end. "
        "use_last_bars or append_song_end only when user wants song-tail extension."
    ),
    "add_dj_drums": (
        "Use get_dj_drum_suggestions for intro/outro/extended-mix params. "
        "placement intro|outro|both; source selection when edit range set."
    ),
    "copy_notes": (
...
def tool_schemas(include_write: bool = True) -> list[dict[str, Any]]:
    schemas: list[dict[str, Any]] = [
        ...
        },
        {
            "type": "function",
            "function": {
                "name": "get_dj_drum_suggestions",
                "description": "Suggested add_dj_drums params for intro, outro, or extended mix (both).",
                "parameters": _object_schema({"song_id": {"type": "string"}}, ["song_id"]),
            },
        },
        {
        ...
        schemas.append(
            {
                ...
                "function": {
                    ...
                    "description": (
                        ...
                        "Use selection master_bar_range for loop_region and add_dj_drums. "
                        "loop_region defaults paste_mode after_selection. "
                        "Extended mix: get_dj_drum_suggestions then add_dj_drums placement both."
                    ),
                    "parameters": _object_schema(
                        {
                            "op_type": {"type": "string"},
...
class ToolExecutor:
    ...
    def execute(self, name: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if name == "get_dj_drum_suggestions":
            return get_dj_drum_suggestions(timeline, args["song_id"])
