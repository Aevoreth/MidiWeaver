...
class AIPlanner:
    SYSTEM_PROMPT = """You are MidiWeaver's arrangement planner. Return ONLY valid JSON matching OperationPlan schema.
...
The user's edit selection is selection.master_bar_range (master timeline bars). Use it for
loop_region source (omit source_* to let the engine fill from selection) and add_dj_drums source selection.

loop_region: song-local source_*_bar or omit for selection; paste_mode after_selection (default) or append_song_end.
add_dj_drums: placement intro|outro|both for extended mixes; style four_on_floor|build_up|phrase_repeat.

tempo_ramp params MUST include start_tick, end_tick, start_bpm, end_bpm, and policy.
echo_notes: use selection.master_bar_range for source ticks when appropriate.
shift_song: song_id plus bars (negative to shift earlier) or delta_ticks."""
