...
from midiweaver.arrangement.dj_drums import apply_dj_drums_to_context
...
class OpExecutor:
    ...
    def validate_op(self, op: Operation) -> list[str]:
        ...
        if op.op_type == "tempo_ramp":
            ...
            if "song_id" not in p:
                errors.append("add_dj_drums missing song_id")
            elif p.get("placement", "outro") not in ("intro", "outro", "both"):
                errors.append("add_dj_drums placement must be intro, outro, or both")
...
def _add_dj_drums(ctx: OpContext, params: dict[str, Any]) -> OpContext:
    return apply_dj_drums_to_context(ctx, params)
...
def _loop_region(ctx: OpContext, params: dict[str, Any]) -> OpContext:
    ...
    for seg in new_ctx.timeline.segments:
        ...
        paste_mode = params.get("paste_mode", "after_selection")
        if paste_mode == "append_song_end":
            dest_base = a.trim_end_tick or src_end
        elif "dest_start_tick" in params:
            dest_base = int(params["dest_start_tick"])
        else:
            dest_base = src_end
