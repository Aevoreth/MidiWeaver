...
def get_loop_candidates(timeline: MasterTimeline, song_id: str) -> dict[str, Any]:
    ...
    return {
        "song_id": song_id,
        "selection_note": (
            "When the UI provides master_bar_range, prefer it over loop_region_params for loop_region "
            "and add_dj_drums source selection."
        ),
...
def get_dj_drum_suggestions(timeline: MasterTimeline, song_id: str) -> dict[str, Any]:
    seg = next((s for s in timeline.segments if s.id == song_id), None)
    if not seg or not seg.analysis:
        return {"error": f"Song not found: {song_id}"}

    a = seg.analysis
    from midiweaver.arrangement.dj_drums import find_drum_track

    drum_track = find_drum_track(a)
    drum_notes = sum(len(t.notes) for t in a.tracks if t.is_drum or t == drum_track)
    bar_count = max(1, int(a.bar_count))
    suggested_bars = 4 if bar_count >= 8 else max(2, bar_count // 2)
    use_template = drum_notes < 8
    style = "four_on_floor" if use_template else "phrase_repeat"
    source = "template" if use_template else "auto"
    ...
    intro_params = {**base, "placement": "intro", "intro_bars": suggested_bars}
    outro_params = {**base, "placement": "outro", "outro_bars": suggested_bars}
    both_params = {
        **base,
        "placement": "both",
        "intro_bars": suggested_bars,
        "outro_bars": suggested_bars,
    }
    return {
        "song_id": song_id,
        ...
        "add_dj_drums_intro": intro_params,
        "add_dj_drums_outro": outro_params,
        "add_dj_drums_both": both_params,
        "note": "Use source selection when UI master_bar_range defines the groove.",
    }
