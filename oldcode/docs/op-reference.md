...
| `loop_region` | Copy a bar range forward (`repeat_count` or `target_total_bars`). Default `paste_mode: after_selection` pastes at end of source; `append_song_end` pastes at song trim end. Source from edit `master_bar_range` when omitted. |
| `add_dj_drums` | DJ-style drum intro/outro (`placement`: intro/outro/both; `style`: four_on_floor, build_up, phrase_repeat) |
